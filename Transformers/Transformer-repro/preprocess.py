...
def download_and_extract(download_dir, url, src_filename, trg_filename):
    ...
    # Check the dataset has been downloaded/extracted properly, otherwise raise an error.
    if src_path and trg_path:
        return src_path, trg_path
...
def main_wo_bpe():
    ...
    if not all([opt.data_src, opt.data_trg]):
        assert {opt.lang_src, opt.lang_trg} == {'de_core_news_sm', 'en_core_web_sm'} # {'de', 'en'}
    else:
